...
st.markdown("ALways Do Prediction First.")
st.divider()

# Define file to store the data
data_file = 'data_frame.joblib' # pca_df_transformed in it it contain dataframe that loaded by joblib 

# Initialize or load the existing dataFrame
if os.path.exists(data_file):
    st.session_state.data = load(data_file)
else:
    st.session_state.data = pd.DataFrame(columns=["PC1", "PC2", "PC3"]) # session_state is a temporay state that has keys like data that has dataframe 
...
try:
    ...
    # Calculate churned and active customers
        ...
        fig, ax = plt.subplots(2, 2, figsize=(12, 10)) # fig is like a sheet and ax is like a plot in it for 2,2 mean 4 plots like 
        # {plot 1 | plot 2
        #  plot 3 | plot 4 }
    ...
    st.write("Please Do Prediction First.")